...
class DataLoader:
  ...
  def load_csv(self):
    ...
    df = df[self.config['Data']['workorderColNames']
            ['WO_DESCRIPTION']].str.lower()
    shorttext_docs = df.tolist()

    # Remove non-alphanumerical characters (except for select few)
    shorttext_docs = [
        re.sub(r'[^0-9A-Za-z-*\/ ]', ' ', str(doc)) for doc in shorttext_docs
    ]
  ...
  def get_stats(self):
    ...
    print(
        f'Average Tokens in MWOs: {sum(doc_lens)/len(doc_lens):0.2f} ({max(doc_lens)} max | {min(doc_lens)} min)'
    )
...
class EOLExpander:
  ...
  def expand_terms(self,
                   ...
                   mode: str = 'replace'):
    '''
      Uses word embeddings to expand a set of terms within derived from a set of maintenance
      work order descriptions docs
    ...
    while active:
      try:
        ...
        def get_similar_terms(term: str, irrelevant_terms: list, topn: int):
          ...
          while len(similar_terms) <= topn:
            similar_terms_temp, similar_terms_proba_temp = zip(
                *model.most_similar([term], topn=topn *
                                    10))    #(topn-len(similar_terms))))
            # remove similar terms already in terms list
            new_terms_idx = [
                idx for idx, similar_term in enumerate(similar_terms_temp)
                if similar_term not in irrelevant_terms
            ]
            # slice terms and proba lists
            similar_terms_temp_slice = np.array(
                list(similar_terms_temp))[new_terms_idx]
            similar_terms_proba_temp_slice = np.array(
                list(similar_terms_proba_temp))[new_terms_idx]
        ...
        for term in terms:
          try:
            similar_terms, similar_terms_proba = get_similar_terms(
                term, irrelevant_terms=terms + irrelevant_terms, topn=topn)
            ...
            if similar_terms is None:
              ...
              print('\t'.join([
                  f'{similar_terms[idx]} ({similar_terms_proba[idx]*100:0.0f}%)'
                  for idx, _ in enumerate(similar_terms)
              ]))
              ...
              if new_terms != '':
                ...
                irrelevant_terms.extend(
                    list(set(similar_terms) - set(new_terms.split())))
              else:
                irrelevant_terms.extend(list(set(similar_terms)))

              print(
                  f'Number of irrelevant terms captured: {len(irrelevant_terms)}'
              )
        ...
        print('*' * 20)
        print(
            f'Added {len(additional_terms)} terms to gazetteer:\n{terms_printout}'
        )
    ...
    print(
        f'Captured {len(additional_terms)} additional terms in {time.time() - start_time:0.0f}s'
    )
...
def controller(config_path: str):
  ...
  # Get new terms
  initial_termsreplace = get_replace_terms()
  terms_listreplace = initial_termsreplace.split()
  dfreplace = expander.expand_terms(model=w2v,
                                    docs=dl.data,
                                    terms=terms_listreplace,
                                    mode='replace')

  # TODO: Determine if keeping this part of the code...
  initial_termsrepair = get_repair_terms()
  terms_listrepair = initial_termsrepair.split()
  dfrepair = expander.expand_terms(model=w2v,
                                   docs=dl.data,
                                   terms=terms_listrepair,
                                   mode='repair')
...
def get_replace_terms():
  return input(
      'Enter initial replace terms (each term separated by a space. hint: replace is a good starting word):\n'
  )


def get_repair_terms():
  return input(
...
def get_new_terms():
  return input('Enter terms to add to list (each term separated by a space):\n')

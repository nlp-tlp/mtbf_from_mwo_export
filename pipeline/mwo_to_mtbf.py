...
class DataLoader:
  ...
  def load_tokens(self):
    ...
    self.token_data = pd.read_excel(
        Path(self.config['File']['outputDir']) / 'token_file.xlsx')
    self.token_data = self.token_data[['Term', 'Action']]

    # Lower-case terms so that they are easier to do sub-string matching against
    self.token_data['Term'] = self.token_data['Term'].apply(
  ...
  def create_token_lists(self):
    ...
    try:
      self.repair_selection = self.token_data[self.token_data['Action'] ==
                                              'repair']['Term'].tolist()
    except Exception as e:
      print(f'{datetime.now()}: Error generating repair selection list - {e}')

    try:
      self.replace_selection = self.token_data[self.token_data['Action'] ==
                                               'replace']['Term'].tolist()
    except Exception as e:
    ...
    self.replace_selection = list(
        set(self.replace_selection).difference(set(self.repair_selection)))
  ...
  def load_workorders(self):
    ...
    if self.workorder_path.split('.')[-1] == 'xlsb':
      ...
      date_cols = [
          self.workorder_col_names['woCreationDate'],
      ...
      for col in date_cols:
        self.mntn_data_all[col] = self.mntn_data_all[col].apply(
            lambda x: date.fromordinal(x + 693594))
    ...
    if self.workorder_path.split('.')[-1] == 'csv':
      self.mntn_data_all = pd.read_csv(self.workorder_path,
                                       encoding='ISO-8859-1')
...
class DataWrangler(DataLoader):
  ...
  def data_preprocess(self):
    ...
    for field in self.workorder_col_names:
      if field == 'additionalCols':
        ...
        self.mntn_data_all.rename(
            columns={self.workorder_col_names[field]: field}, inplace=True)
    ...
    if self.workorder_col_names['additionalCols'] is not None:
      self.mntn_data = self.mntn_data_all[
          col_subset + self.workorder_col_names['additionalCols']]
    else:
      self.mntn_data = self.mntn_data_all[col_subset]

    # Enforce data type on datetime columns
    self.mntn_data[[
        'CREATION_DATE', 'BASIC_START_DATE', 'BASIC_FINISH_DATE',
    ...
    self.mntn_data['WO_DESCRIPTION'] = self.mntn_data['WO_DESCRIPTION'].apply(
        lambda x: re.sub(r'[^0-9A-Za-z-*\/ ]', ' ', str(x)))
    ...
    doc_lens = [
        len(x.split()) for x in self.mntn_data['WO_DESCRIPTION'].tolist()
    ]

    print(
        f'{datetime.now()}: Mean token length - {sum(doc_lens)/len(doc_lens)}')
    ...
    if self.filters['dropNA']:
      ...
      # Actual Start Time
      self.mntn_data.dropna(subset=['ACTUAL_START_DATE'],
                            how='all',
                            inplace=True)
      # Cost, Time or Cost AND Time
      if set(['TOTAL_ACTUAL_HOURS',
              'TOTAL_ACTUAL_COST']).issubset(set(self.mntn_data.columns)):
        print(f'{datetime.now()}: Dataset has both cost and time')
        self.mntn_data.dropna(
            subset=['TOTAL_ACTUAL_HOURS', 'TOTAL_ACTUAL_COST'],
            how='all',
            inplace=True)
      elif 'TOTAL_ACTUAL_HOURS' in self.mntn_data.columns:
        print(f'{datetime.now()}: Dataset has only time')
        self.mntn_data.dropna(subset=['TOTAL_ACTUAL_HOURS'],
                              how='all',
                              inplace=True)
      elif 'TOTAL_ACTUAL_COST' in self.mntn_data.columns:
        print(f'{datetime.now()}: Dataset has only cost')
        self.mntn_data.dropna(subset=['TOTAL_ACTUAL_COST'],
                              how='all',
                              inplace=True)
      else:
        # No structured data
    ...
    # Filter based on creation date (in particular for CMMS transitions which aggregate historical
    # WOs on a point transition time)
    if self.filters['dateLower'] is not None:
      self.mntn_data = self.mntn_data[(self.filters['dateLower'] <=
                                       self.mntn_data['CREATION_DATE'])]

    # Filter based on particular asset/object type
    if self.filters['objectType']:
      print(
          f'{datetime.now()}: Filtering dataset for object/asset types - {self.filters["objectType"]}'
      )
      self.mntn_data = self.mntn_data[self.mntn_data['OBJECT_TYPE'].apply(
          lambda x: str(x).lower()).str.contains(self.filters['objectType'])]

    print(
        f'{datetime.now()}: Original Size: {start_size} -> Filtered Size: {len(self.mntn_data)}'
    )
  ...
  def return_to_function(self):
    ...
    if self.cmms_type == 'JDE':
      activity_type_list = [
          'REPAIR', 'OVERHAUL', 'REBUILD'
      ]    # Note: JDE doesn't have REPLACE; This is JDE specific, not an available field in SAP/1SAP

      # Filter out repair events. TODO: Make robust for cost OR time
      if 'TOTAL_ACTUAL_HOURS' in self.mntn_data.columns:
        self.mntn_data = self.mntn_data.drop(
            self.mntn_data[(self.mntn_data['ACTIVITY_TYPE'] == 'REPAIR') &
                           (self.mntn_data['TOTAL_ACTUAL_HOURS'] <
                            self.filters['hourMin'])].index)
      else:
        self.mntn_data = self.mntn_data.drop(
            self.mntn_data[(self.mntn_data['ACTIVITY_TYPE'] == 'REPAIR')].index)

      self.mntn_data['RETURN_TO_FUNCTION'] = np.where(
          ((self.mntn_data['ACTIVITY_CAUSE'] == 'PREVENTIVE') &
           (self.mntn_data['ACTIVITY_TYPE'].isin(activity_type_list))),
          'PROACTIVE',
          np.where(((self.mntn_data['ACTIVITY_CAUSE'] == 'CORRECTIVE') &
                    (self.mntn_data['ACTIVITY_TYPE'].isin(activity_type_list))),
                   'REACTIVE', np.nan))
    ...
    if self.cmms_type in ['SAP', '1SAP']:
      ...
      if 'TOTAL_ACTUAL_COST' in self.mntn_data.columns:
        self.mntn_data = self.mntn_data.drop(
            self.mntn_data[(self.mntn_data['TOTAL_ACTUAL_COST'] <
                            self.filters['costMin'])].index)
      else:
        pass

      # Assuming that PM01/PM02/PM03 are perfect indicators of state changing events...
      self.mntn_data['RETURN_TO_FUNCTION'] = np.where(
          (self.mntn_data['WO_CLASSIFICATION'] == 'PM02'), 'PROACTIVE',
          np.where((self.mntn_data['WO_CLASSIFICATION'] == 'PM01') |
                   (self.mntn_data['WO_CLASSIFICATION'] == 'PM03'), 'REACTIVE',
                   np.nan))
  ...
  def token_match(self):
    ...
    self.mntn_data['REPAIR_OR_REPLACE'] = np.where(
        self.mntn_data['WO_DESCRIPTION'].apply(
            lambda x: any(item for item in self.repair_selection if item in x)),
        'REPAIR',
        np.where(
            self.mntn_data['WO_DESCRIPTION'].apply(lambda x: any(
                item for item in self.replace_selection if item in x)),
            'REPLACE', np.nan))
  ...
  def identify_fs(self):
    ...
    try:
      self.mntn_data['FAILURE_OR_SUSPENSION'] = np.where(
          (self.mntn_data['RETURN_TO_FUNCTION'] == 'REACTIVE') &
          (self.mntn_data['REPAIR_OR_REPLACE'] == 'REPLACE'), 'FAILURE',
          np.where((self.mntn_data['RETURN_TO_FUNCTION'] == 'PROACTIVE') &
                   (self.mntn_data['REPAIR_OR_REPLACE'] == 'REPLACE'),
                   'SUSPENSION', np.nan))
    except Exception as e:
      print(
          f'{datetime.now()}: Error identifying failure and suspensions - {e}')
...
class ParameterEstimator(DataWrangler):
  ...
  def preprocess_fs_data(self):
    ...
    # Group by asset and time, sort newest to oldet (reference is ACTUAL_START_DATE).
    # TODO: Validate the date is correct (was CREATION_DATE)
    self.mntn_data = self.mntn_data.sort_values(
        by=['FUNCTIONAL_LOC', 'ACTUAL_START_DATE'], ascending=True)

    # Create a mask for only F or S data (exlcudes NaN)
    fs_mask = ((self.mntn_data['FAILURE_OR_SUSPENSION'] == 'FAILURE') |
               (self.mntn_data['FAILURE_OR_SUSPENSION'] == 'SUSPENSION'))
    ...
    self.mntn_data_fs['TIME'] = self.mntn_data_fs.groupby(
        'FUNCTIONAL_LOC')['ACTUAL_START_DATE'].diff() / np.timedelta64(1, 'D')
    self.mntn_data_fs['TIME'] = self.mntn_data_fs['TIME'].fillna(0)

    # Add Time data to main dataframe for output later on
    self.mntn_data = pd.concat([self.mntn_data, self.mntn_data_fs['TIME']],
                               axis=1)
    # Round Time
    self.mntn_data['TIME'] = self.mntn_data['TIME'].apply(
        lambda x: x if math.isnan(x) else int(x))

    # Subset dataframe keeping only columns necessary for Weibull calculations
    self.mntn_data_fs_ss = self.mntn_data_fs[[
        'FUNCTIONAL_LOC', 'FAILURE_OR_SUSPENSION', 'TIME'
    ]]
    ...
    self.mntn_data_fs_ss_counts = self.mntn_data_fs_ss.groupby([
        'FUNCTIONAL_LOC'
    ])['FAILURE_OR_SUSPENSION'].agg(['count', count_failure, count_suspension])
  ...
  def calculate_mtbf(self):
    ...
    fs_data['FAILURE_OR_SUSPENSION'].replace(to_replace='FAILURE',
                                             value=1,
                                             inplace=True)
    fs_data['FAILURE_OR_SUSPENSION'].replace(to_replace='SUSPENSION',
                                             value=0,
                                             inplace=True)
    ...
    fs_data_ss = fs_data_ss.groupby(
        ['FUNCTIONAL_LOC']).filter(lambda x: self.no_data_points <= len(x))
    ...
    def compute_single_mtbf(series):
      ...
      series = series[0 < series[
          'TIME']]    # do not take into account any events that have 0 time...
    ...
    self.mtbf_data[['MTBF', 'BETA', 'ETA']] = fs_data_ss.groupby(
        ['FUNCTIONAL_LOC']).apply(lambda grp: compute_single_mtbf(grp))
  ...
  def save_results(self):
    ...
    output['OVERVIEW'] = {
        'TOTAL_WORKORDERS':
            len(self.mntn_data),
        'FAILURES':
            int(self.mntn_data_fs_ss_counts['count_failure'].sum()),
        'SUSPENSIONS':
            int(self.mntn_data_fs_ss_counts['count_suspension'].sum()),
        'TOTAL_ASSETS':
            self.mntn_data['FUNCTIONAL_LOC'].nunique(
            ),    # number of assets that went through the filters etc.
        'MTBF_ASSETS':
            self.mtbf_data.index.nunique(
            )    # number of assets that had MTBFs calculated
    }
    ...
    for floc in self.mtbf_data.index:
      output['RESULTS'][floc] = {
          'MTBF':
              round(self.mtbf_data.loc[floc]['MTBF'], 2),
          'ETA':
              round(self.mtbf_data.loc[floc]['ETA'], 2),
          'BETA':
              round(self.mtbf_data.loc[floc]['BETA'], 2),
          'COUNTS': {
              'FAILURE':
                  int(self.mntn_data_fs_ss_counts.loc[floc]['count_failure']),
              'SUSPENSION':
                  int(self.mntn_data_fs_ss_counts.loc[floc]['count_suspension']
                     ),
              'TOTAL':
                  len(self.mntn_data[self.mntn_data['FUNCTIONAL_LOC'] == floc])
          },
          'WO_DESCRIPTIONS_FS':
              self.mntn_data[(self.mntn_data['FUNCTIONAL_LOC'] == floc) &
                             (self.mntn_data['FAILURE_OR_SUSPENSION'] != 'nan')]
              [[
                  'WO_DESCRIPTION', total_cost_or_time_col, 'WO_CLASSIFICATION',
                  'RETURN_TO_FUNCTION', 'REPAIR_OR_REPLACE',
                  'FAILURE_OR_SUSPENSION', 'TIME'
              ]].to_dict(),
          'WO_DESCRIPTION_NFS':
              self.mntn_data[(self.mntn_data['FUNCTIONAL_LOC'] == floc) &
                             (self.mntn_data['FAILURE_OR_SUSPENSION'] == 'nan')]
              [[
                  ...
                  'RETURN_TO_FUNCTION', 'REPAIR_OR_REPLACE',
                  'FAILURE_OR_SUSPENSION', 'TIME'
              ]].to_dict()    # Filtered using NaN in F/S column
      }

    with open(
        Path(self.config['File']['outputDir']) / 'mtbf_results.json',
        'w') as fp:
      json.dump(output, fp, indent=4)

    if self.config['File']['saveCSV']:
      cost_or_hours = 'hours' if self.config['Data'].get(
          'TOTAL_ACTUAL_HOURS') else 'cost'
      json2csv(
          Path(self.config['File']['outputDir']) / 'mtbf_results.json',
          Path(self.config['File']['outputDir']) / 'mtbf_results.csv',
          cost_or_hours)
...
def controller(config_path: str):
  ...
  print(
      f'{datetime.now()}: Process took - {(time.time() - start_time):0.2f}s to complete.'
  )
